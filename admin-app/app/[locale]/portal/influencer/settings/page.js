...
function Toggle({ value, onChange }) {
  return (
    <button onClick={() => onChange(!value)} type="button"
      style={{ width: 44, height: 24, borderRadius: 12, border: 'none', background: value ? '#8b5cf6' : '#d1d5db', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
...
export default function InfluencerSettingsPage() {
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      const [nRes, pRes] = await Promise.all([
        influencerApi('/notifications'),
        influencerApi('/privacy'),
      ]);
  ...
  const saveNotifs = async () => {
    setSaving(true);
    try {
      await influencerApi('/notifications', { method: 'PUT', body: notifs });
  ...
  const savePrivacy = async () => {
    setSaving(true);
    try {
      await influencerApi('/privacy', { method: 'PUT', body: privacy });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {tab === 'notifications' && notifs && (
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e5e7eb', padding: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {[
              { key: 'newOffers',        label: 'New Offers',           desc: 'Get notified when new offers match your profile' },
              { key: 'applicationUpdates', label: 'Application Updates', desc: 'Updates on your application status' },
              { key: 'payments',         label: 'Payments',             desc: 'Payment received and payout notifications' },
              { key: 'messages',         label: 'Messages',             desc: 'New messages from brands' },
          ...
          <button onClick={saveNotifs} disabled={saving}
            style={{ marginTop: 20, padding: '10px 24px', borderRadius: 10, border: 'none', background: saving ? '#a5b4fc' : 'linear-gradient(135deg, #8b5cf6, #7c3aed)', color: '#fff', fontSize: 13, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer' }}>
      ...
      {tab === 'privacy' && privacy && (
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e5e7eb', padding: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {[
              { key: 'profileVisible',    label: 'Public Profile',       desc: 'Allow brands to discover your profile' },
              { key: 'showFollowers',     label: 'Show Followers',       desc: 'Display follower count on your profile' },
              { key: 'showEarnings',      label: 'Show Earnings',        desc: 'Display earnings on your public profile' },
              { key: 'allowMessages',     label: 'Allow Messages',       desc: 'Allow brands to send you direct messages' },
